parSRGS<-function(data, dropout, p = 0.8, iter = 10, k = 1, stepsize = 0.01, num.cores = 1){
  
   
  cl <- makeCluster(num.cores) 
  clusterExport(cl,c("data","dropout","p","iter","k","stepsize","spls"),envir=environment())
  ...
  for ( i in 1:ncol(data))
  {
    ...
    res<-parLapply(cl,seq(0.01,0.99,stepsize),function(x,dataX,dataY,k,dropout,p,stepsize,iter){
      set.seed(x*100)
      try({
        if(dropout==FALSE)
        {
